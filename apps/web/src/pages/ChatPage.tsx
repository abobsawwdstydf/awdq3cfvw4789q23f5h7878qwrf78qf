...
export default function ChatPage() {
  ...
  const [streamChatName, setStreamChatName] = useState('');
  ...
  // Mobile: switch to chat view when activeChat changes
  useEffect(() => {
    if (activeChat) {
  ...
  const handleStartStream = (chatId: string, chatName: string) => {
    setStreamChatId(chatId);
    setStreamChatName(chatName);
    setStreamSessionId(id => id + 1);
    setStreamOpen(true);
  };
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="h-full flex bg-surface p-2 sm:p-3 gap-2 sm:gap-3 overflow-hidden"
    >
      {/* Sidebar - hidden on mobile when viewing chat */}
      <div className={`${showMobileList ? 'flex' : 'hidden'} md:flex flex-shrink-0 w-full md:w-auto`}>
      ...
      <LiveStreamModal
        key={`ls-${streamSessionId}`}
        isOpen={streamOpen}
        onClose={() => setStreamOpen(false)}
        chatId={streamChatId}
        chatName={streamChatName}
      />
