...
export default function Sidebar({ onChatSelect }: SidebarProps) {
  ...
  return (
    <>
      <div className="w-full md:w-[340px] h-full flex flex-col bg-surface-secondary rounded-2xl md:rounded-3xl overflow-hidden border border-border/50 shadow-2xl relative z-10">
        {/* Шапка */}
        ...
        <div className="flex-1 overflow-y-auto">
          {filteredChats.length === 0 ? (
            ...
            filteredChats.map((chat) => (
              <ChatListItem key={chat.id} chat={chat} isActive={chat.id === activeChat} onChatSelect={onChatSelect} />
            ))
          )}
        </div>
      </div>
