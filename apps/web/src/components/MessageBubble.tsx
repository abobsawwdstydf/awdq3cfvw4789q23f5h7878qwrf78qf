...
function MessageBubble({
  ...
  const hasVideoNote = message.type === 'videoNote' || media.some((m) => m.type === 'videoNote');
  const hasAudio = !hasVoice && !hasVideoNote && (message.type === 'audio' || media.some((m) => m.type === 'audio'));
  const hasFile = media.some((m) => m.type !== 'image' && m.type !== 'voice' && m.type !== 'video' && m.type !== 'videoNote' && m.type !== 'audio');
          ...
          >
            ...
            {hasVoice && (
              <div className="flex items-center gap-2.5 min-w-[200px]">
                <audio
                  ref={audioRef}
                  src={media.find((m) => m.type === 'voice')?.url || undefined}
                  preload="metadata"
                  onError={(e) => {
                    const audioEl = e.target as HTMLAudioElement;
                    console.warn('Audio load error (may be loading):', audioEl.src);
                  }}
                />
                {/* Play button - circular */}
                <button
                  onClick={toggleAudio}
                  className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 shadow-md transition-all hover:scale-105 ${
                    isMine 
                      ? 'bg-white/20 hover:bg-white/30' 
                      : 'bg-Nimbus-500/20 hover:bg-Nimbus-500/30'
                  }`}
                >
                  {isPlaying ? (
                    <Pause size={18} className={isMine ? 'text-white' : 'text-Nimbus-400'} />
                  ) : (
                    <Play size={18} className={`${isMine ? 'text-white' : 'text-Nimbus-400'} ml-0.5`} />
                  )}
                </button>
                <div className="flex-1 min-w-0">
                  ...
                  <span className={`text-xs mt-1 block font-mono ${isMine ? 'text-white/60' : 'text-zinc-500'}`}>
                    {isPlaying
            ...
            {/* Видеокружок - Telegram style */}
            {hasVideoNote ? (() => {
              const videoNoteMedia = media.find((m) => m.type === 'videoNote');
              return (
                <div className="flex items-center gap-2.5">
                  <VideoNotePlayer
                    src={videoNoteMedia?.url}
                    duration={videoNoteMedia?.duration || 0}
                    filename={videoNoteMedia?.filename || undefined}
                  />
                </div>
              );
            })() : null}
...
function AlbumView({ media, contentExists, isMine, onImageClick }: AlbumViewProps) {
  ...
  return (
    <div className={`${contentExists ? 'mb-2 -mx-3 -mt-2' : ''} ${!contentExists ? 'rounded-[1.25rem]' : ''} bg-black/40 overflow-hidden`}>
      ...
      {/* Media grid */}
      <div className={`grid ${getGridClass()} gap-0.5 ${!isExpanded && isAlbum ? 'max-h-48 overflow-hidden' : ''}`}>
        {media.map((m, idx) => (
          <div
            key={m.id}
            className={`relative aspect-square overflow-hidden cursor-pointer group ${!isExpanded && isAlbum && idx >= 3 ? 'hidden' : ''}`}
...
function VideoPlayer({ src, isMine, filename, onOpenLightbox }: VideoPlayerProps) {
  ...
  return (
    <div
      className={`relative rounded-xl overflow-hidden bg-black group ${!isFullscreen ? 'max-h-40 sm:max-h-80' : ''}`}
    ...
    >
      ...
      {/* Play/Pause button (center) - larger and more visible */}
      {!isPlaying && !isLoading && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <motion.div
            initial={{ scale: 0.8, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            className="w-20 h-20 rounded-full bg-black/70 backdrop-blur-md flex items-center justify-center shadow-2xl border-2 border-white/20"
          >
      ...
      <AnimatePresence>
        {showControls && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-black/40 flex flex-col justify-end p-3 sm:p-4"
          >
            {/* Top bar - filename and fullscreen */}
            <div className="flex items-center justify-between mb-3 sm:mb-4">
              {filename && (
                <span className="text-white/80 text-xs sm:text-sm truncate max-w-[60%] hidden sm:block">{filename}</span>
              )}
              <button
            ...
            <div
              className="w-full h-1.5 bg-white/20 rounded-full cursor-pointer group/progress mb-4 overflow-hidden"
              onClick={handleProgressClick}
            >
              <div
                className={`h-full rounded-full relative transition-all ${isMine ? 'bg-white' : 'bg-Nimbus-400'}`}
...
// ─── Video Note Player (Telegram-style circular video) ────────────────────────
...
function VideoNotePlayer({ src, duration, filename }: VideoNotePlayerProps) {
  const videoRef = useRef<HTMLVideoElement>(null);
  const [isPlaying, setIsPlaying] = useState(false);
  const [progress, setProgress] = useState(0);
  ...
  return (
    <div className="flex items-center gap-2.5">
      {/* Circular video - Telegram style */}
      <div
        className="relative w-14 h-14 rounded-full overflow-hidden flex-shrink-0 cursor-pointer bg-black shadow-lg"
        onClick={togglePlay}
      >
        <video
          ref={videoRef}
        ...
        {/* Play overlay - centered in circle */}
        {!isPlaying && !isLoaded && (
        ...
        {!isPlaying && isLoaded && (
          <div className="absolute inset-0 bg-black/30 flex items-center justify-center">
            <div className="w-8 h-8 rounded-full bg-white/90 flex items-center justify-center hover:bg-white transition-colors shadow-lg">
      ...
      <div className="flex-1 min-w-0">
        {/* Progress bar */}
        <div
          className="relative w-full h-1 bg-white/10 rounded-full cursor-pointer overflow-hidden group"
          onClick={handleSeek}
        >
          <div
            className="absolute inset-y-0 left-0 bg-Nimbus-400 rounded-full transition-all duration-100"
            style={{ width: `${progress}%` }}
          />
          {/* Seek handle */}
          <div
        ...
        <span className="text-xs text-white/60 mt-1 block font-mono">
